...
`result` — 0 успех, 1 предупреждение, 2 ошибка. `last_success_unix` — время последнего
**успешного** прогона; при отказе `backup.bat` переносит его из предыдущего маркера, не
обнуляя. Без этого одна ночная неудача стирала бы всю историю успеха, и вместо «успех был
26 часов назад» приходило бы «успеха не было ни разу». `finished_unix` — время окончания в
...
**Файл должен оставаться чисто ASCII.** Windows PowerShell 5.1 — а на psvc есть только он —
читает файл без BOM как ANSI/cp1251, и одного кириллического байта в комментарии достаточно,
чтобы скрипт превратился в мешанину и сломался на строковом литерале: `The string is missing
the terminator`. Задача при этом возвращает 1 и не пишет ничего — то есть выглядит как
исправный коллектор, не нашедший данных. Поймано 12.09.2026 при первой же установке. У
`veeam-metrics.ps1` комментарии английские, потому там это и не стреляло.
...
## backup.bat и write-status.ps1

Ночные дампы PostgreSQL и маркер состояния, который читает `pgdump-metrics.ps1`.
Развёрнутые копии живут в `C:\pg_dump\` на psvc.

Здесь они оказались 12.09.2026 по той же причине, что и всё остальное в этом
каталоге: **единственная копия скрипта бэкапа лежала на той самой машине, которую
он бэкапит.** Упади она — и восстанавливать пришлось бы по памяти, ровно в тот
момент, когда не до того.

### Вызов

```
backup.bat <пользователь> <база> <хост> ["дополнительные опции pg_dump"]
```

Четвёртый аргумент передаётся в `pg_dump` как есть. Еженедельный дамп Zabbix
использует его, чтобы сохранить схему, но выбросить данные истории:
`"--exclude-table-data=history* --exclude-table-data=trends*"` — 3.9 МБ вместо
10.3 ГБ, и восстановленная база поднимается рабочей с пустыми графиками.

### Что здесь заслуживает внимания

**Успех — это код возврата ноль И ненулевой размер.** Прежняя версия проверяла
только существование файла, и десять дней подряд писала в журнал
`completed successfully` для пустого архива в 264 байта. Отсюда же проверка
`ENCSIZE LSS 1024` после шифрования.

**Единственная точка выхода `:done`.** Маркер пишется при любом исходе, включая
ранние отказы, и обязательно **пока `Q:` ещё смонтирован** — иначе размер
архива взять неоткуда: шара подключается только на время задачи. Размер
промежуточного файла не сказал бы ничего, он-то как раз создавался исправно.

**`last_success_unix` переносится при неудаче, а не обнуляется.** Одна неудачная
ночь не должна превращать «последний успех сутки назад» в «успеха не было
никогда» — второе не срабатывает по правилу возраста вообще.

**Секретов в аргументах нет.** Пароль базы приходит через `PGPASSFILE`, фраза
GPG — через `--passphrase-file`, учётные данные шары — из `secrets.cmd`. Все три
файла доступны только `locadmin` и `SYSTEM`. До 12.09.2026 пароль базы
передавался аргументом задачи планировщика, то есть был виден любому, кто может
прочитать список задач.

`STORAGEBOX_HOST` — заглушка: репозиторий публичный, развёрнутая копия несёт
настоящий адрес.

### Известные шероховатости

`net use` получает пароль шары аргументом командной строки. На время выполнения
он виден в списке процессов. Обойти это в cmd нечем; вариант — заранее
сохранённые учётные данные Windows, но тогда они перестают быть видимыми в
репозитории и появляется своя проблема.

Чистка хранит 10 дней архивов **в общем каталоге**. Для суточных задач это
десять копий, для недельного дампа Zabbix — обычно одна. Для единственного
бэкапа конфигурации тонковато; стоит либо увеличить глубину, либо развести
задачи по каталогам.

`backup.bat` коммитится с CRLF и без BOM: cmd спотыкается на BOM, а `write-status.ps1`
чисто ASCII, и BOM ему не нужен — но помните, что скрипт PowerShell с кириллицей
и без BOM на этой машине ломается (см. раздел `pgdump-metrics.ps1`).
